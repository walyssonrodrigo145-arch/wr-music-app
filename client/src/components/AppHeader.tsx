  ...
  return (
    <header className="h-16 lg:h-20 bg-white border-b border-slate-100 flex items-center px-4 lg:px-8 gap-4 lg:gap-6 flex-shrink-0 z-10 sticky top-0">
      ...
      <div ref={searchRef} className="relative hidden md:flex items-center w-40 lg:w-80">
        <Search size={16} className="absolute left-4 text-slate-400 pointer-events-none z-10" />
        <input
          value={searchQuery}
          onChange={e => { setSearchQuery(e.target.value); setSearchOpen(true); }}
          onFocus={() => setSearchOpen(true)}
          placeholder="Procurar..."
          className="w-full h-10 lg:h-11 pl-10 lg:pl-12 pr-10 text-xs font-bold bg-[#F8FAFC] border-none rounded-2xl focus:bg-white focus:ring-2 focus:ring-blue-500/20 transition-all text-slate-800 placeholder:text-slate-400 shadow-sm"
        />
        {searchQuery && (
        ...
        <AnimatePresence>
          {searchOpen && searchQuery.trim().length >= 2 && (
            <motion.div 
              ...
              exit={{ opacity: 0, y: 10 }}
              className="absolute top-14 left-0 right-0 bg-white border border-slate-100 rounded-2xl shadow-2xl z-50 overflow-hidden"
            >
      ...
      <div className="flex items-center gap-2 lg:gap-3">
        ...
        <button className="w-10 h-10 rounded-xl bg-white border border-slate-100 text-slate-400 hover:text-slate-800 hover:bg-slate-50 transition-all shadow-sm flex items-center justify-center active:scale-95" onClick={toggleTheme}>
          {theme === "dark" ? <Sun size={18} /> : <Moon size={18} />}
        </button>

        <button className="w-10 h-10 rounded-xl bg-white border border-slate-100 text-slate-400 hover:text-[#2563EB] hover:bg-slate-50 transition-all shadow-sm relative flex items-center justify-center active:scale-95" onClick={() => navigate("/configuracoes")}>
          <Bell size={18} />
