...
| Repertório (YouTube) | `server/routers/repertoireRouters.ts` + `server/utils/youtubeUrl.ts` (parser puro — iframe usa SÓ videoId validado) + `components/progresso/RepertoireTab.tsx` (professor) + `components/student/RepertoireSection.tsx` (portal) | montar iframe com URL crua do usuário; o aluno nunca edita título/URL (só viewed/learned próprios) |
