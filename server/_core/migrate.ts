...
export async function runAutoMigrations() {
  ...
  try {
    ...
    const migrations = [
      ...
      { table: 'professores', sql: 'ALTER TABLE "professores" ALTER COLUMN "permissions" SET DEFAULT \'["/dashboard", "/alunos", "/aulas"]\'::jsonb' },
      ...
      // Snapshots de tempo real (serviço independente de Analytics) + correção de drift
      ...
      { table: 'enrollment_links', sql: `ALTER TABLE "enrollment_links" ADD COLUMN IF NOT EXISTS "contractTemplateId" integer` },
      { table: 'enrollment_links', sql: `ALTER TABLE "enrollment_links" ADD COLUMN IF NOT EXISTS "teacherUserId" integer` },
      { table: 'students', sql: `ALTER TABLE "students" ADD COLUMN IF NOT EXISTS "guardianCpf" varchar(20)` },
      ...
      { table: 'support_tickets', sql: `ALTER TABLE "support_tickets" ADD COLUMN IF NOT EXISTS "attachments" text` },
      { table: 'support_tickets', sql: `ALTER TABLE "support_tickets" ADD COLUMN IF NOT EXISTS "hasUnreadResponse" boolean DEFAULT false NOT NULL` },
      { table: 'student_enrollments', sql: `CREATE TABLE IF NOT EXISTS "student_enrollments" ("id" serial PRIMARY KEY, "organizationId" integer NOT NULL, "studentId" integer NOT NULL, "instrumentId" integer, "planId" integer, "teacherUserId" integer, "studioRoomId" integer, "durationMonths" integer DEFAULT 1 NOT NULL, "lessonsPerWeek" integer DEFAULT 1 NOT NULL, "weekday" integer DEFAULT 1 NOT NULL, "timeStr" varchar(5), "monthlyFee" decimal(10,2) DEFAULT '0.00' NOT NULL, "enrollmentFee" decimal(10,2) DEFAULT '0.00' NOT NULL, "startDate" date, "endDate" date, "status" varchar(20) DEFAULT 'ativo' NOT NULL, "createdAt" timestamp default now() NOT NULL, "updatedAt" timestamp default now() NOT NULL)` },
      { table: 'student_enrollments', sql: `CREATE INDEX IF NOT EXISTS "student_enrollments_student_idx" ON "student_enrollments" ("studentId")` },
      { table: 'student_enrollments', sql: `CREATE INDEX IF NOT EXISTS "student_enrollments_org_idx" ON "student_enrollments" ("organizationId")` },
      { table: 'message_automation_rules', sql: `
      ...
      // ── PROGRAMA INDIQUE & GANHE (indicação de escolas) ──────────────────────
      { table: 'referral_config', sql: `CREATE TABLE IF NOT EXISTS "referral_config" (
        "id" integer PRIMARY KEY DEFAULT 1,
        "active" boolean DEFAULT true NOT NULL,
        "trialDays" integer DEFAULT 7 NOT NULL,
        "rewardMode" varchar(20) DEFAULT 'PROGRESSIVO' NOT NULL,
        "cycleSize" integer DEFAULT 3 NOT NULL,
        "rewardPercent1" integer DEFAULT 30 NOT NULL,
        "rewardPercent2" integer DEFAULT 60 NOT NULL,
        "rewardPercent3" integer DEFAULT 100 NOT NULL,
        "fixedValueCents" integer DEFAULT 0 NOT NULL,
        "percentValue" integer DEFAULT 0 NOT NULL,
        "maxDiscountPercent" integer DEFAULT 100 NOT NULL,
        "allowAccumulation" boolean DEFAULT true NOT NULL,
        "rewardValidityDays" integer DEFAULT 90 NOT NULL,
        "minActiveDays" integer DEFAULT 0 NOT NULL,
        "blockSelfReferral" boolean DEFAULT true NOT NULL,
        "pageHeadline" varchar(255) DEFAULT 'Sua escola foi indicada!' NOT NULL,
        "pageSubtitle" text,
        "updatedByUserId" integer,
        "createdAt" timestamp DEFAULT now() NOT NULL,
        "updatedAt" timestamp DEFAULT now() NOT NULL
      );` },
      { table: 'referral_codes', sql: `CREATE TABLE IF NOT EXISTS "referral_codes" (
        "id" serial PRIMARY KEY NOT NULL,
        "organizationId" integer NOT NULL UNIQUE,
      ...
      { table: 'referral_codes', sql: `CREATE INDEX IF NOT EXISTS "referral_codes_code_idx" ON "referral_codes" ("code")` },
      { table: 'referrals', sql: `CREATE TABLE IF NOT EXISTS "referrals" (
        "id" serial PRIMARY KEY NOT NULL,
        "code" varchar(20) NOT NULL,
        "referrerOrgId" integer NOT NULL,
        "referredOrgId" integer NOT NULL UNIQUE,
        "status" varchar(30) DEFAULT 'PENDENTE' NOT NULL,
        "cyclePosition" integer DEFAULT 0 NOT NULL,
        "rewardPercent" integer DEFAULT 0 NOT NULL,
        "planValueCents" integer DEFAULT 0 NOT NULL,
        "indicatedAt" timestamp DEFAULT now() NOT NULL,
        "signupAt" timestamp,
      ...
      { table: 'referrals', sql: `CREATE INDEX IF NOT EXISTS "referrals_referrer_idx" ON "referrals" ("referrerOrgId")` },
      { table: 'referrals', sql: `CREATE INDEX IF NOT EXISTS "referrals_referred_idx" ON "referrals" ("referredOrgId")` },
      { table: 'referrals', sql: `CREATE INDEX IF NOT EXISTS "referrals_status_idx" ON "referrals" ("status")` },
      { table: 'referrals', sql: `CREATE INDEX IF NOT EXISTS "referrals_code_idx" ON "referrals" ("code")` },
      { table: 'referral_rewards', sql: `CREATE TABLE IF NOT EXISTS "referral_rewards" (
        "id" serial PRIMARY KEY NOT NULL,
        "referralId" integer NOT NULL,
        "organizationId" integer NOT NULL,
        "type" varchar(20) DEFAULT 'PERCENTUAL' NOT NULL,
        "percent" integer DEFAULT 0 NOT NULL,
        "fixedValueCents" integer DEFAULT 0 NOT NULL,
        "appliedValueCents" integer DEFAULT 0 NOT NULL,
