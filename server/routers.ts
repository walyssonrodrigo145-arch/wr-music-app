...
"${input.text}"`;
  ...
  crm: router({
    getDashboardMetrics: protectedProcedure.query(async ({ ctx }) => {
      ...
      const leadsList = await db.select().from(crmLeads).where(eq(crmLeads.organizationId, orgId));

      const totalLeads = leadsList.length;
      const activeLeads = leadsList.filter(l => l.stage !== 'fechado' && l.stage !== 'perdido').length;
      const demosCount = leadsList.filter(l => l.stage === 'demonstracao').length;
      const proposalsCount = leadsList.filter(l => l.stage === 'proposta').length;
      const negotiationsCount = leadsList.filter(l => l.stage === 'negociacao').length;
      const closedDeals = leadsList.filter(l => l.stage === 'fechado').length;

      const newMrr = leadsList
        .filter(l => l.stage === 'fechado')
        .reduce((sum, l) => sum + Number(l.value || 0), 0);

      const conversionRate = totalLeads > 0 ? (closedDeals / totalLeads) * 100 : 0;

      // Agrupamento por origem (Origem dos Leads)
      ...
      const sourcesList = Object.entries(sourcesMap).map(([name, count]) => ({
        name,
        count,
        percentage: totalLeads > 0 ? Math.round((count / totalLeads) * 100) : 0,
      })).sort((a, b) => b.count - a.count);

      // Agrupamento por estágio para o Funil de Vendas
      const stageMap: Record<string, { label: string; count: number }> = {
        novo: { label: "Leads recebidos", count: 0 },
        contato: { label: "Contatados", count: 0 },
        interessado: { label: "Interessados", count: 0 },
        demonstracao: { label: "Demonstrações", count: 0 },
        proposta: { label: "Propostas", count: 0 },
        negociacao: { label: "Negociações", count: 0 },
        fechado: { label: "Clientes (Fechado)", count: 0 },
      };
      ...
      const funnelList = Object.entries(stageMap).map(([stageKey, item]) => ({
        stageKey,
        label: item.label,
        count: item.count,
        percentage: totalLeads > 0 ? ((item.count / totalLeads) * 100).toFixed(1) : "0.0",
      }));
    ...
    createLead: protectedProcedure
      .input(z.object({
        ...
        instrument: z.string().optional(),
        planName: z.string().optional().default("Plano Pro"),
        stage: z.string().optional().default("novo"),
        temperature: z.string().optional().default("quente"),
        value: z.string().optional().default("199.00"),
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const [created] = await db.insert(crmLeads).values({
          organizationId: orgId,
          name: input.name,
          companyOrSchool: input.companyOrSchool || null,
          cityState: input.cityState || null,
          phone: input.phone || null,
          email: input.email || null,
          instrument: input.instrument || null,
          planName: input.planName || "Plano Pro",
          stage: input.stage || "novo",
          temperature: input.temperature || "quente",
          value: input.value || "199.00",
    ...
    moveLeadStage: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const [updated] = await db.update(crmLeads)
          ...
          .where(and(eq(crmLeads.id, input.id), eq(crmLeads.organizationId, orgId)))
          .returning();
    ...
    getGoals: protectedProcedure.query(async ({ ctx }) => {
      ...
      let [goal] = await db.select()
        .from(crmGoals)
        .where(and(eq(crmGoals.organizationId, orgId), eq(crmGoals.monthYear, currentMonthYear)))
        .limit(1);
    ...
    saveGoal: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (existing) {
          const [updated] = await db.update(crmGoals)
            .set({ ...input, updatedAt: new Date() })
            .where(eq(crmGoals.id, existing.id))
            .returning();
          return updated;
        } else {
          const [created] = await db.insert(crmGoals).values({
    ...
    listActivities: protectedProcedure.query(async ({ ctx }) => {
      ...
      if (list.length === 0) {
        // Exemplo padrão idêntico ao layout
        return [
          { id: 1, title: "Follow-up: Escola Vivace", type: "whatsapp", description: "Retornar contato via WhatsApp", scheduledTime: "09:30", assignedUserName: "Walysson" },
          { id: 2, title: "Ligação: Center Music", type: "call", description: "Confirmar demonstração", scheduledTime: "11:00", assignedUserName: "Irmão" },
          { id: 3, title: "Demonstração: Adagio", type: "demo", description: "Apresentação do sistema", scheduledTime: "14:00", assignedUserName: "Walysson" },
          { id: 4, title: "Proposta: Escola Allegro", type: "proposal", description: "Enviar proposta comercial", scheduledTime: "15:30", assignedUserName: "Irmão" },
          { id: 5, title: "Follow-up: Compasso", type: "whatsapp", description: "Negociação em andamento", scheduledTime: "16:30", assignedUserName: "Walysson" },
        ];
      }
