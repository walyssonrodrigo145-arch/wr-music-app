...
[FIM DO TEMPLATE]`;
  ...
  reminders: router({
    ...
    // ─ Excluir lembrete ────────────────────────────────────────────────────────────
    // Regra: lembretes gerados automaticamente (autoGenerated=1) NUNCA são deletados do banco.
    // Ao "excluir", eles são marcados como 'enviado' — isso serve como trava permanente
    // para o robô não recriar o mesmo lembrete no próximo ciclo.
    // Apenas lembretes manuais (autoGenerated=0) são deletados de verdade.
    delete: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // Busca o lembrete para verificar se é automático
        const [reminder] = await db.select({ id: reminders.id, autoGenerated: reminders.autoGenerated })
          .from(reminders)
          .where(and(eq(reminders.id, input.id), eq(reminders.organizationId, orgId), eq(reminders.userId, ctx.user.id)))
          .limit(1);
        ...
        if (reminder.autoGenerated === 1) {
          // Lembrete automático: marca como 'enviado' para bloquear regeneração pelo robô
          await db.update(reminders)
            .set({ status: "enviado", sentAt: new Date(), errorMessage: "Arquivado pelo usuário.", updatedAt: new Date() })
            .where(eq(reminders.id, reminder.id));
        } else {
          // Lembrete manual: delete real
