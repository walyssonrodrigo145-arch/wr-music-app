...
// AUDIT-07 FIX: a função agora é async e semeia a janela com o nº real de lembretes
// enviados na última hora (tabela reminders). Assim, após um restart do container
// (deploy), o contador NÃO zera — o limite anti-ban continua valendo.
async function canSendWhatsApp(userId: number): Promise<boolean> {
  const now = Date.now();
  const entry = whatsappSentByUser.get(userId);
  if (!entry || (now - entry.windowStart) > 60 * 60 * 1000) {
    // Janela nova (1 hora) — semeia com envios reais da última hora
    let seeded = 0;
    try {
      ...
      if (db) {
        ...
        const rows = await db.select({ count: sql<number>`count(*)` })
          ...
          .where(and(
            ...
            gte(reminders.sentAt, oneHourAgo)
          ));
        seeded = Number(rows[0]?.count ?? 0);
      }
    } catch (e) {
      debugLog(`[ANTI-BAN] Falha ao semear janela de rate limit do usuário ${userId}:`, String(e));
    }
    const count = seeded + 1;
    whatsappSentByUser.set(userId, { count, windowStart: now });
    if (count > MAX_WA_PER_HOUR) {
      console.warn(`[ANTI-BAN] Usuário ${userId} já enviou ${seeded} msgs na última hora (pós-restart). Limite de ${MAX_WA_PER_HOUR}/hora mantido. Envio bloqueado.`);
      return false;
    }
    return true;
  }
          ...
          try {
            if (rule.trigger === "payment_due" || rule.trigger === "payment_overdue") {
              ...
              for (const due of pendingDues) {
                ...
                if (userSet.whatsappAutoSend === 1 && userSet.whatsappBotUrl && (await canSendWhatsApp(userId))) {
              ...
              for (const payment of confirmedToday) {
                if (!payment.allowAutoReminders) continue;
